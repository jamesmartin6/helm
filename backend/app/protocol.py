...
Identical to sil-harness/protocol.py -- kept as a separate copy since the
backend and SiL harness are independent Python projects/venvs; the spec
itself (this docstring + firmware/src/protocol.h) is the shared source of
truth, so keep both copies in sync if the wire format ever changes.
...
# DTC bitmask values (active_dtc_mask in the telemetry frame), mirrored from
